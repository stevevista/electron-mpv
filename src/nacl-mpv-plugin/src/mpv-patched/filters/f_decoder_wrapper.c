...
#include "detection/detection-service.h"
#endif
...
const struct m_sub_options dec_wrapper_conf = {
    .opts = (const struct m_option[]){
        ...
        {"audio-reversal-buffer", OPT_BYTE_SIZE(audio_reverse_size),
            M_RANGE(0, M_MAX_MEM_BYTES)} ,
#ifdef MPV_CUSTOMIZE_USE_HOT_DETECTION
        {"hot-detection", OPT_FLAG(hot_detection)},
#endif
        {0}
    },
...
void demuxer_feed_detection(struct sh_stream *stream, demux_packet_t *dp);
...
static void on_detections(void *opaque, const ai_detection_t * dets, int count) {
    detection_ctx_t *ctx = (detection_ctx_t*)opaque;
    struct priv *p = ctx->p;
    double pts = ctx->pts;
    double dts = ctx->dts;
    free(ctx);
    ...
    if (!dets) count = 0;
    size_t len = sizeof(ai_detection_t) * count;

    av_new_packet(&pkt, len + 4);
    memcpy(pkt.data, "HOTD", 4);
    if (count) memcpy(pkt.data + 4, (void*)dets, len);

    struct demux_packet *ccpkt = new_demux_packet_from_avpacket(&pkt);
    ccpkt->pts = pts;
    ccpkt->dts = dts;
    // demuxer_feed_detection(p->header, ccpkt);
...
#ifdef MPV_CUSTOMIZE_USE_HOT_DETECTION
        if (p->opts->hot_detection) {
            AVFrame *pic = av_frame_alloc();
            if (pic) {
                for (int n = 0; n < 4; n++) {
                    pic->data[n] = mpi->planes[n];
                    pic->linesize[n] = mpi->stride[n];
                }
                pic->format = imgfmt2pixfmt(mpi->imgfmt);
                pic->width = mpi->w;
                pic->height = mpi->h;
                
                detection_ctx_t *ctx = malloc(sizeof(detection_ctx_t));
                ctx->p = p;
                ctx->pts = mpi->pts;
                ctx->dts = mpi->dts;
                int r = netwrok_async_detection(&p->detect_id, pic, on_detections, ctx);
                if (r < 0) {
                    free(ctx);
                }
                av_frame_free(&pic);
...
#ifdef MPV_CUSTOMIZE_USE_HOT_DETECTION
    netwrok_async_detection_cancel(p->detect_id);
#endif
