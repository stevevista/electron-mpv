...
static void assobjects_init(struct sd *sd)
{
    ...
    if (strcmp(sd->codec->codec, "eia_608") == 0) {
        static const char *extradata = 
            ...
            "Style: AIRender,Courier New,20,&Hffffff,&Hffffff,&H0,&Hffffffff,0,0,0,0,100,100,0,0,4,0,1,7,0,0,0,0\n"
            "[Events]\n"
            "Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n\0";
        
        ass_process_codec_private(ctx->ass_track, extradata, strlen(extradata));
        ass_set_check_readorder(ctx->ass_track, 0);
...
static int init(struct sd *sd)
{
    ...
    {
#ifndef MPV_CUSTOMIZE_USE_HOT_DETECTION
...
static void decode(struct sd *sd, struct demux_packet *packet)
{
    ...
    if (strcmp(sd->codec->codec, "eia_608") == 0) {
        ...
        if (size >= 4 && !memcmp(data, "HOTD", 4)) {
            long long newStart = llrint(packet->pts * 1000);
            for (int n = 0; n < track->n_events - 1; n++) {
                if (track->events[n].Start < newStart) {
                        track->events[n].Duration = newStart -
                                                    track->events[n].Start - 1;
                }
            }
            ...
            for (int n = 0; n < count; n++) {
                char *lines[8];
                int ev_count = ai_render_detection_to_ass_lines(
                    1024,
                    768,
                    locale,
                    &detections[n],
